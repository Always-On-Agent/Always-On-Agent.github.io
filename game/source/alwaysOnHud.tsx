...
const words = {
  en: {
    ...
    controls: 'WASD move · drag / mouse to look · E interact · H glasses',
  ...
  zh: {
    ...
    controls: 'WASD 移动 · 拖动 / 鼠标转头 · E 交互 · H 眼镜',
...
export const alwaysOnHudState = proxy({
  ready: false, language: initialLanguage(), visit: 1, quiet: false,
...
export function isAlwaysOnTargetVisible (player: Bot, target: Position, maxDistance = 5.5): boolean {
...
export function setupAlwaysOnDemo (player: Bot, options: AlwaysOnSceneOptions) {
  ...
  Object.assign(alwaysOnHudState, { ready: true, visit: memory.visits + 1, quiet: memory.quiet, rememberedNotice: memory.knewNotice, noticeObserved: false, complete: false, receipt: '', target: '', busy: false, error: '', journal: memory.journal })
  ...
  document.addEventListener('keydown', event => {
    ...
    if (event.code === 'KeyH' && (isGameActive(true) || activeModalStack.at(-1)?.reactType === MODAL)) {
