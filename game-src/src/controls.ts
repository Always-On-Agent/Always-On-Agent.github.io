...
import { alwaysOnMenuInput, isAlwaysOnScene, shouldHandleAlwaysOnTab } from './alwaysOnMenuInput'
...
export const contro = new ControMax({
  ...
  target: document,
  captureEvents (e) {
    // Tab belongs to the scene cursor toggle; in forms/dialogs it keeps native focus traversal.
    if (e?.code === 'Tab' && miscUiState.gameLoaded && isAlwaysOnScene()) return false
...
contro.on('movementUpdate', ({ vector, soleVector, gamepadIndex }) => {
  if (isAlwaysOnScene() && alwaysOnMenuInput.cursorReleased) return
...
const onTriggerOrReleased = (command: Command, pressed: boolean) => {
  ...
  if (pressed && isAlwaysOnScene() && alwaysOnMenuInput.cursorReleased && (command.startsWith('movement.') || command.startsWith('general.rotateCamera'))) return
...
document.addEventListener('keydown', (event) => {
  if (!isAlwaysOnScene() || !miscUiState.gameLoaded || !shouldHandleAlwaysOnTab(event, Boolean(pointerLock.hasPointerLock), activeModalStack.length > 0)) return
  if (!pointerLock.hasPointerLock && isNextConsoleKeyboardTarget(event)) return
  event.preventDefault()
  event.stopImmediatePropagation()
  if (event.repeat) return
  if (pointerLock.hasPointerLock) {
    alwaysOnMenuInput.suppressPause()
    alwaysOnMenuInput.noteCursorRelease()
    // Keyup may be lost when pointer lock ends, so explicitly release the input library too.
    for (const code of contro.pressedKeys) contro.pressedKeyOrButtonChanged({ code }, false)
    cameraRotationControls.activeDirections.clear()
    cameraRotationControls.updateMovement()
    bot?.clearControlStates()
    bot?.mouse?.buttons?.fill(false)
    gameAdditionalState.isSprinting = false
    gameAdditionalState.isSneaking = false
    tabListState.isOpen = false
    document.exitPointerLock?.()
    window.dispatchEvent(new Event('always-on-cursor-released'))
  } else {
    pointerLock.justHitEscape = false
...
addEventListener('mousedown', async (e) => {
  ...
  // HUD controls keep an ordinary cursor; only a scene click captures the mouse.
  if ((e.target as HTMLElement).closest?.('button, a, input, select, textarea, [contenteditable="true"], [role="dialog"], .ao-interactive')) return
...
window.addEventListener('keydown', (e) => {
  ...
  if (isAlwaysOnScene()) {
    e.preventDefault()
    if (!alwaysOnMenuInput.noteKeyboardEscape(e.repeat)) return
    if (activeModalStack.length) {
      alwaysOnMenuInput.suppressPause()
      if (e.ctrlKey || e.metaKey) hideAllModals()
      else hideCurrentModal()
      if (!activeModalStack.length) pointerLock.justHitEscape = true
    } else if (miscUiState.gameLoaded && !appViewer.rendererState.preventEscapeMenu) {
      showModal({ reactType: 'pause-screen' })
      alwaysOnMenuInput.suppressPause()
      document.exitPointerLock?.()
    }
