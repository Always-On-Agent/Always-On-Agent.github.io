...
import { campusPlaceCategory, searchCampusPlaces } from './ntuPlaceSearch'
...
type TourStop = { id: string; name: SingaporeText; position: MinimapPoint; aliases?: string[]; category?: string; mapVisible?: boolean; navigationNote?: string | SingaporeText }
type Props = { player: MapPlayer; stations: readonly Station[]; tourStops?: readonly TourStop[]; language: SingaporeLanguage; focus: SingaporeStation; touch: boolean; onVisit?: (id: string) => void }
const CAMPUS_MAP_MODAL = 'singapore-campus-map'
const tourSymbols: Record<string, string> = { 'tour-nie': 'N', 'tour-residences': 'R', 'tour-sports': 'S', 'tour-adm': 'A', 'tour-hive': 'H', 'tour-north-spine': 'NS', 'tour-south-spine': 'SS', 'tour-wave': 'W', 'tour-heritage': 'HC', 'tour-gaia': 'G', 'tour-arc': 'AR', 'tour-halls-3-16': '3/16', 'tour-lake': 'LK' }
...
function loadMap () {
  mapRequest ??= fetch(ntuMapAsset('minimap.json')).then(async response => {
...
type CampusMapProps = Pick<Props, 'player' | 'stations' | 'tourStops' | 'language' | 'focus' | 'onVisit'> & {
  map?: MapData; raster?: HTMLCanvasElement; unavailable: boolean; onClose: () => void
}

function CampusMapDialog ({ player, stations, tourStops = [], language, focus, map, raster, unavailable, onClose, onVisit }: CampusMapProps) {
  const dialogRef = useRef<HTMLElement>(null)
  const canvasRef = useRef<HTMLCanvasElement>(null)
  const viewRef = useRef<MinimapView>()
  const dragRef = useRef<{ x: number; y: number; startX: number; startY: number; moved: boolean; pointer: number }>()
  const [camera, setCamera] = useState<{ zoom: number; center?: MinimapPoint }>({ zoom: 1 })
  const zh = language === 'zh'
  const places = useMemo(() => [...stations, ...tourStops] as TourStop[], [stations, tourStops])
  const [query, setQuery] = useState('')
  const [selectedId, setSelectedId] = useState('')
  const matches = useMemo(() => searchCampusPlaces(places, query), [places, query])
  const mapPlaces = query ? matches : places.filter(place => place.mapVisible !== false)
  const selectedPlace = places.find(place => place.id === selectedId)
  const choosePlace = (place: TourStop) => { setSelectedId(place.id); focusPoint(place.position) }
  const visitPlace = () => { if (selectedPlace && onVisit) { onClose(); onVisit(selectedPlace.id) } }
  ...
  useEffect(() => {
    ...
    const draw = (time: number) => {
      ...
      for (const station of mapPlaces) {
        ...
        const story = station.id in stationSymbols; const selected = station.id === selectedId || (!selectedId && station.id === focus)
        context.beginPath(); context.arc(point.x, point.y, selected ? 11 : camera.zoom < 2 && mapPlaces.length > 50 ? 3 : 7.5, 0, Math.PI * 2)
        ...
        if (selected || mapPlaces.length < 50 || camera.zoom >= 3) { context.fillStyle = '#304940'; context.font = '700 9px system-ui'; context.textAlign = 'center'; context.textBaseline = 'middle'; context.fillText(symbol(station.id), point.x, point.y + .3) }
      }
      if (player.entity) {
  ...
  }, [map, raster, player, stations, tourStops, camera, focus, query, selectedId])
  return <div className='sg-campus-map-layer' onPointerDown={event => event.stopPropagation()} onMouseDown={event => event.stopPropagation()} onClick={event => { event.stopPropagation(); if (event.target === event.currentTarget) onClose() }}>
    <section ref={dialogRef} className='sg-campus-map-dialog' role='dialog' aria-modal='true' aria-label={zh ? 'NTU 全校园地图' : 'NTU full campus map'} tabIndex={-1}>
      ...
      <div className='sg-campus-place-search'><input type='search' value={query} onChange={event => setQuery(event.target.value)} aria-label={zh ? '搜索校园地点' : 'Search campus places'} placeholder={zh ? '搜索楼宇、宿舍、房间… 如 Maple、Hall 1' : 'Search buildings, halls, rooms… e.g. Maple, Hall 1'} /><span>{matches.length} {zh ? '个地点' : 'places'}</span></div>
      <div className='sg-campus-map-surface'>
        <canvas ref={canvasRef} role='img' aria-label={zh ? 'NTU 全校园地理底图，显示实时位置、探索地点和剧情地点' : 'Full NTU geographic map with live position and story and exploration places'} onPointerDown={event => {
          if (event.button !== 0) return
          event.preventDefault(); event.currentTarget.setPointerCapture(event.pointerId)
          dragRef.current = { x: event.clientX, y: event.clientY, startX: event.clientX, startY: event.clientY, moved: false, pointer: event.pointerId }
        }} onPointerMove={event => {
          const drag = dragRef.current; const view = viewRef.current
          if (!drag || drag.pointer !== event.pointerId || !view || !map) return
          ...
          if (!drag || drag.moved || !viewRef.current) return
          const bounds = event.currentTarget.getBoundingClientRect(); const x = event.clientX - bounds.left; const y = event.clientY - bounds.top
          const nearest = mapPlaces.map(place => { const pixel = minimapPixel(place.position, viewRef.current!, bounds.width, bounds.height); return { place, distance: Math.hypot(pixel.x - x, pixel.y - y) } }).sort((a, b) => a.distance - b.distance)[0]
          if (nearest && nearest.distance <= 20) choosePlace(nearest.place)
      ...
      {selectedPlace && <div className='sg-campus-selected'><div><strong>{name(selectedPlace)}</strong><span>{campusPlaceCategory(selectedPlace.category, language)} · {typeof selectedPlace.navigationNote === 'object' ? selectedPlace.navigationNote[language] : zh ? '前往附近已检查的室外落点' : 'Arrive at a checked outdoor point nearby'}</span></div>{onVisit && <button onClick={visitPlace}>{zh ? '前往这里' : 'Go here'} ↗</button>}</div>}
      <nav className='sg-campus-map-places' aria-label={zh ? '地图地点' : 'Map places'}>{matches.slice(0, 40).map(station => <button key={station.id} aria-pressed={selectedId === station.id} onClick={() => choosePlace(station)}><i className={station.id === focus ? 'is-focus' : station.id in stationSymbols ? 'is-story' : 'is-tour'}>{symbol(station.id)}</i><span>{name(station)}<small>{campusPlaceCategory(station.category, language)}</small></span></button>)}</nav>
      <footer><span>{matches.length > 40 ? (zh ? `显示前 40 个结果；输入名称缩小范围。` : 'Showing 40 results; search to narrow the list.') : (zh ? '点击地图标记或地点名称，再选择前往。' : 'Select a map marker or place, then Go here.')}</span><a href='https://www.openstreetmap.org/copyright' target='_blank' rel='noreferrer'>© OpenStreetMap contributors · ODbL</a></footer>
    </section>
  </div>
}

export default function SingaporeMinimap ({ player, stations, tourStops = [], language, focus, touch, onVisit }: Props) {
  const canvasRef = useRef<HTMLCanvasElement>(null)
  ...
  return <><figure className={`sg-minimap${touch ? ' sg-minimap-touch' : ''}`} aria-label={language === 'zh' ? 'NTU 实时小地图，北朝上；箭头表示当前位置与朝向' : 'Live NTU minimap, north up; arrow shows your position and heading'}>
    ...
    <button ref={openButtonRef} className='sg-minimap-expand' aria-haspopup='dialog' aria-expanded={expanded} aria-label={language === 'zh' ? '展开全校园地图' : 'Open full campus map'} title={language === 'zh' ? '展开全校园地图' : 'Open full campus map'} onPointerDown={event => event.stopPropagation()} onMouseDown={event => event.stopPropagation()} onClick={event => {
      event.stopPropagation()
      showModal({ reactType: CAMPUS_MAP_MODAL })
      document.exitPointerLock?.()
    }}><span aria-hidden='true'>⤢</span></button>
  </figure>
  {expanded && createPortal(<CampusMapDialog player={player} stations={stations} tourStops={tourStops} language={language} focus={focus} map={mapData} raster={rasterRef.current} unavailable={unavailable} onClose={closeMap} onVisit={onVisit} />, document.body)}
  </>
}
