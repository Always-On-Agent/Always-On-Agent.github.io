...
import { eccvHudState } from '../eccvHud'
import { isSingaporeScene } from '../alwaysOnScene'
import { singaporeHudState } from '../singaporeHud'
...
export default () => {
  const { language } = useSnapshot(isSingaporeScene ? singaporeHudState : eccvHudState)
  ...
  return <div style={{
    ...
    {captureMouseHint && !usingTouch && !usingGamepadInput && !acitveModals && inRealGameSession && <div style={{
      ...
      <div>{language === 'zh' ? '点击画面，开始探索' : 'Click the scene to explore'}</div>
    </div>}
  </div>
}
