...
type LifeBot = { entity?: { position: LifePoint }; blockAt?: (point: Vec3) => unknown; once?: (event: 'end', callback: () => void) => unknown; removeListener?: (event: 'end', callback: () => void) => unknown }
...
export function installSingaporeLife (renderer: LifeRenderer, bot: LifeBot, scene: LifeScene = {}) {
  ...
  void fetch(ntuMapAsset('routes.json'), { signal: controller.signal }).then(async response => {
