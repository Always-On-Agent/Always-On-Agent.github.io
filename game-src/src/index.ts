...
import { installEccvWorld, ECCV_SCENE } from './eccvWorld'
import { setupEccvDemo } from './eccvHud'
import { isSingaporeScene } from './alwaysOnScene'
import { loadSingaporeMap, getSingaporeScene, installSingaporeMap } from './singaporeMap'
import { setupSingaporeDemo } from './singaporeHud'
import { createSingaporeActions } from './singaporeActions'
import { installSingaporeMarkers } from './singaporeMarkers'
import { createCampusArchitecture } from './ntuCampusArchitecture'
import { createHallPrototypeDecorator, getLoadedHallPrototype } from './ntuHallPrototype'
import { isNtuMapRebuild } from './ntuMapLocal'
import { installSingaporeLife } from './singaporeLife'
import { installEccvSigns } from './eccvSigns'
...
export async function connect (connectOptions: ConnectOptions) {
  ...
  try {
    ...
    if (singleplayer) {
      ...
      if (isSingaporeScene) {
        const registry = require('minecraft-data')(getSingaporeScene().version)
        const decorateCampus = isNtuMapRebuild() ? undefined : createCampusArchitecture(registry, getSingaporeScene().architectureOffset)
        const prototype = getLoadedHallPrototype()
        const decorateHall = prototype && new URLSearchParams(location.search).get('prototypeView') !== 'baseline'
          ? createHallPrototypeDecorator(registry, prototype) : undefined
        installSingaporeMap(localServer, { decorateChunk: (chunk, x, z) => {
          decorateCampus?.(chunk, x, z)
          decorateHall?.(chunk, x, z)
        } })
      } else installEccvWorld(localServer)
  ...
  const displayWorld = async () => {
    ...
    try {
      ...
      if (isSingaporeScene) {
        ...
        await bot.look(scene.spawn.yaw, 0, true)
        const actions = createSingaporeActions(localServer, bot, [...scene.stations, ...(scene.tourStops ?? [])])
        setupSingaporeDemo(bot, { stations: scene.stations, tourStops: scene.tourStops, ...actions })
        const removeMarkers = installSingaporeMarkers(window.world, scene.stations)
        const removeCampusLife = installSingaporeLife(window.world, bot, scene)
        bot.once('end', removeMarkers)
        bot.once('end', removeCampusLife)
      } else {
        await bot.look(ECCV_SCENE.spawn.yaw, 0, true)
        setupEccvDemo(bot, { posters: ECCV_SCENE.posters, reportPoint: ECCV_SCENE.reportPoint })
        const removeEccvSigns = installEccvSigns(window.world)
        bot.once('end', removeEccvSigns)
      }
      window.parent.postMessage({ type: 'always-on-game-ready' }, location.origin)
...
const maybeEnterGame = () => {
  ...
  // Each exhibit starts directly, with no account or world-selection menus.
  return waitForConfigFsLoad(() => {
    if (isSingaporeScene) {
      void loadSingaporeMap().catch(error => {
        console.error('NTU map loading failed', error)
        setLoadingScreenStatus(new URLSearchParams(location.search).get('lang') === 'zh' ? 'NTU 地图加载失败，请刷新重试。' : 'NTU map loading failed. Please refresh to retry.')
      })
      return
    }
    loadSingleplayer({
      generation: { name: 'empty', options: {} },
      gameMode: 2,
      difficulty: 0,
      'view-distance': 3,
      'max-entities': 0,
  ...
  if (appQueryParams.connectPeer) {
    ...
    const username = (options.askGuestName
      ? prompt('Enter your username to connect to peer', options.guestUsername)
  ...
  const requestedModals = appQueryParams.modal ?? ''
  if (requestedModals) {
    const modals = requestedModals.split(',')
