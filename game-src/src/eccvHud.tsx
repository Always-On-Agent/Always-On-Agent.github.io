...
function toggleWalking () {
  ...
  if (!activeBot || !eccvHudState.ready || !isGameActive(true) || document.hidden || alwaysOnMenuInput.cursorReleased) return
...
export function openEccvPanel (tab: Tab = 'card') {
  ...
  stopMovement(); eccvHudState.tab = tab; showModal({ reactType: MODAL }); alwaysOnMenuInput.suppressPause(); document.exitPointerLock?.()
}
...
export function setupEccvDemo (player: ScenePlayer, options: EccvSceneOptions) {
  ...
  document.addEventListener('keydown', event => {
    ...
    const top = activeModalStack.at(-1)?.reactType
    if (event.code === 'Escape' && (!top || top === MODAL || top === 'pause-screen')) {
      event.preventDefault(); event.stopImmediatePropagation()
      if (!alwaysOnMenuInput.noteKeyboardEscape(event.repeat)) return
      stopMovement()
      if (top === MODAL) closePanel(false)
      else { if (top === 'pause-screen') hideModal(activeModalStack.at(-1)); openEccvPanel() }
      return
    }
  ...
  window.addEventListener('always-on-cursor-released', stopMovement, { signal: abort.signal })
