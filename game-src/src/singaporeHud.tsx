...
import NtuHallPrototypeHud from './ntuHallPrototypeHud'
import NtuCampusExploreHud from './ntuCampusExploreHud'
import NtuRegionBanner from './ntuRegionBanner'
import NtuCompanionHud from './ntuCompanionHud'
import { getLoadedHallPrototype } from './ntuHallPrototype'
import { isNtuMapRebuild, ntuMapAsset, ntuMemoryKey } from './ntuMapLocal'
import './alwaysOnHud.css'
import './singaporeHud.css'

type Position = { x: number; y: number; z: number }
type ScenePlayer = Pick<Bot, 'entity' | 'controlState' | 'blockAt' | 'clearControlStates' | 'setControlState' | 'once' | 'on' | 'look' | 'removeListener' | 'mouse'> & { world?: unknown }
...
export type SingaporeSceneOptions = {
  ...
  tourStops?: ReadonlyArray<{ id: string; name: SingaporeText; position: Position; approach: Position & { yaw: number }; category?: string; aliases?: string[]; navigationNote?: string | SingaporeText; sourceId?: string; buildingId?: string; mapVisible?: boolean }>
  acceptLoan: () => Promise<Result>
  returnLoan: () => Promise<Result>
  collectKit: () => Promise<Result>
  shuttle: (id: string, signal?: AbortSignal) => Promise<Result>
  observer?: (mode: 'fly' | 'overhead' | 'walk') => Promise<Result>
}
...
const DAY_MODAL = 'ntu-day-demo'
const MAP_MODAL = 'singapore-campus-map'
const NtuDayDemo = lazy(() => import('./ntuDayDemo'))
const NtuLiveEpisode = lazy(async () => import('./ntuLiveEpisode'))
const words = {
  en: {
    ...
    controls: 'WASD move · Tab cursor · M map · Esc panel', mobile: 'Use the touch controls; tap Map or the nearby prompt.',
  ...
  zh: {
    ...
    mapSources: '地图来源',
    controls: 'WASD 移动 · Tab 鼠标 · M 地图 · Esc 面板', mobile: '使用触控移动，点击地图或附近信息站提示。',
...
export function getSingaporeMemory (): SingaporeMemory {
  ...
  try {
    const data = JSON.parse(localStorage.getItem(ntuMemoryKey(SINGAPORE_STORY.memoryKey)) ?? 'null')
...
export const singaporeHudState = proxy({
  ...
  observerMode: 'walk' as 'walk' | 'fly' | 'overhead', tab: 'now' as Tab, journal: [] as Entry[], travelDone: false, scriptedDay: false
})
let observerView: ReturnType<typeof createNtuObserverView> | undefined
...
function persist () {
  ...
  try { localStorage.setItem(ntuMemoryKey(SINGAPORE_STORY.memoryKey), JSON.stringify({ visits: state.visit, accepted: state.accepted, quiet: state.quiet, bookReceipt: state.bookReceipt, meetupReceipt: state.meetupReceipt, kitReceipt: state.kitReceipt, pickupAccepted: state.pickupAccepted, knewNotice: state.knewNotice || state.noticeObserved, knewPickup: state.knewPickup || state.pickupObserved, journal: state.journal.slice(-60) })) } catch {}
}
...
function toggleWalking () {
  ...
  if (!activeBot || !singaporeHudState.ready || !isGameActive(true) || document.hidden || singaporeHudState.busy || singaporeHudState.observerMode !== 'walk' || alwaysOnMenuInput.cursorReleased) return
  stopWalking()
...
function closePanel (_resume = false) {
...
function openCampusMap () {
  stopWalking()
  closePanel(false)
  showModal({ reactType: MAP_MODAL })
  alwaysOnMenuInput.suppressPause(); document.exitPointerLock?.()
}
function openDayDemo () {
  if (!isNtuMapRebuild() || !scene || singaporeHudState.busy || singaporeHudState.observerMode !== 'walk') return
  closePanel(false)
  const map = activeModalStack.find(value => value.reactType === MAP_MODAL)
  if (map) hideModal(map)
  stopWalking()
  singaporeHudState.scriptedDay = false
  showModal({ reactType: DAY_MODAL })
  alwaysOnMenuInput.suppressPause(); document.exitPointerLock?.()
}
function closeDayDemo () {
  const modal = activeModalStack.find(value => value.reactType === DAY_MODAL)
  if (modal) hideModal(modal)
  stopWalking()
}
function refreshSceneCamera () {
  const entity = activeBot?.entity
  if (entity && typeof appViewer !== 'undefined') appViewer.backend?.updateCamera(entity.position, entity.yaw, entity.pitch, { instant: true })
}
async function travelDayDemo (id: string, signal: AbortSignal): Promise<boolean> {
  const currentScene = scene
  const player = activeBot
  if (!currentScene || !player || signal.aborted) return false
  stopWalking()
  const result = await currentScene.shuttle(id, signal)
  if (result.ok && !signal.aborted && scene === currentScene) refreshSceneCamera()
  return !signal.aborted && scene === currentScene && activeBot === player && result.ok
}
...
function takeShuttle (id: string) {
  if (!scene || singaporeHudState.observerMode !== 'walk') return
  const currentScene = scene
  void transaction(async () => currentScene.shuttle(id), () => {
    ...
    refreshSceneCamera()
    log('shuttle', 'A → M')
    if (isNtuMapRebuild()) window.parent.postMessage({ type: 'always-on-campus-place', id }, location.origin)
    closePanel()
  })
}
async function changeObserverMode (mode: 'walk' | 'fly' | 'overhead') {
  const currentScene = scene
  if (!currentScene?.observer || singaporeHudState.busy) return
  stopWalking()
  singaporeHudState.target = ''
  singaporeHudState.busy = true
  singaporeHudState.error = ''
  try {
    const result = await currentScene.observer(mode)
    if (scene !== currentScene) return
    if (result.observerMode) singaporeHudState.observerMode = result.observerMode
    else if (result.ok) singaporeHudState.observerMode = mode
    await observerView?.setMode(singaporeHudState.observerMode)
    if (!result.ok) singaporeHudState.error = 'failed'
  } catch { if (scene === currentScene) singaporeHudState.error = 'failed' } finally {
    if (scene === currentScene) singaporeHudState.busy = false
  }
}
function interact () {
...
export function setupSingaporeDemo (player: ScenePlayer, options: SingaporeSceneOptions) {
  previousCleanup?.()
  activeBot = player
  if (isNtuMapRebuild() && typeof appViewer !== 'undefined') observerView = createNtuObserverView(player, appViewer)
  scene = options
  const memory = getSingaporeMemory()
  Object.assign(singaporeHudState, { ready: true, observerMode: 'walk', visit: memory.visits + 1, accepted: memory.accepted, quiet: memory.quiet, bookReceipt: memory.bookReceipt, meetupReceipt: memory.meetupReceipt, kitReceipt: memory.kitReceipt, pickupAccepted: memory.pickupAccepted, knewNotice: memory.knewNotice, knewPickup: memory.knewPickup, journal: memory.journal, walking: false, target: '', focus: 'plaza', busy: false, restoring: false, restoreFailed: false, error: '', tab: 'now', noticeObserved: false, pickupObserved: false, travelDone: false })
  if (memory.accepted) log('restored', 'M')
  focusNext()
  persist()
  const abort = new AbortController()
  // Renderer switches carry a named place, never arbitrary teleport coordinates.
  const initialPlace = isNtuMapRebuild() ? new URLSearchParams(location.search).get('campusPlace') : null
  if (initialPlace && options.tourStops?.some(stop => stop.id === initialPlace)) {
    singaporeHudState.busy = true
    void options.shuttle(initialPlace, abort.signal).then(result => {
      if (abort.signal.aborted || scene !== options) return
      if (result.ok) {
        refreshSceneCamera()
        window.parent.postMessage({ type: 'always-on-campus-place', id: initialPlace }, location.origin)
      } else { singaporeHudState.error = 'failed' }
    }).catch(() => { if (!abort.signal.aborted && scene === options) singaporeHudState.error = 'failed' }).finally(() => {
      if (!abort.signal.aborted && scene === options) singaporeHudState.busy = false
    })
  }
  const observer = setInterval(() => {
    if (!isGameActive(true) || document.hidden || singaporeHudState.observerMode !== 'walk') return
  ...
  document.addEventListener('keydown', event => {
    ...
    const topModal = activeModalStack.at(-1)?.reactType
    if (topModal === DAY_MODAL) return
    if (event.code === 'Escape' && (!topModal || [MODAL, MAP_MODAL, 'pause-screen'].includes(topModal))) {
      event.preventDefault(); event.stopImmediatePropagation()
      if (!alwaysOnMenuInput.noteKeyboardEscape(event.repeat)) return
      stopWalking()
      if (topModal === MODAL) closePanel(false)
      else if (topModal === MAP_MODAL) hideModal(activeModalStack.at(-1))
      else { if (topModal === 'pause-screen') hideModal(activeModalStack.at(-1)); openSingaporePanel() }
      return
    }
    if ((event.target as HTMLElement)?.closest('input, textarea, select, [contenteditable="true"]')) return
    if (event.code === 'KeyM' && (!topModal || topModal === MODAL || topModal === MAP_MODAL)) {
      event.preventDefault(); event.stopImmediatePropagation()
      if (topModal === MAP_MODAL) hideModal(activeModalStack.at(-1)); else openCampusMap()
      return
    }
  ...
  window.addEventListener('always-on-cursor-released', stopWalking, { signal: abort.signal })
  window.addEventListener('blur', stopWalking, { signal: abort.signal })
  window.addEventListener('message', event => {
    if (event.source !== window.parent || event.origin !== location.origin || event.data?.type !== 'always-on-game-mode') return
    if (!event.data.active) {
      stopWalking()
      alwaysOnMenuInput.suppressPause(); document.exitPointerLock?.()
      window.dispatchEvent(new Event('always-on-demo-pause'))
    }
  }, { signal: abort.signal })
  ...
  const cleanup = () => {
    ...
    closePanel(false)
    closeDayDemo()
    observerView?.dispose(); observerView = undefined
...
export default function SingaporeHud () {
  ...
  const dayOpen = useIsModalActive(DAY_MODAL)
  ...
  if (dayOpen) return <Suspense fallback={<div className='ntu-demo-loading'>Loading…<button onClick={closeDayDemo}>←</button></div>}>{state.scriptedDay || !activeBot
    ? <NtuDayDemo language={state.language} onExit={closeDayDemo} onTravel={travelDayDemo} />
    : <NtuLiveEpisode language={state.language} bot={activeBot} onExit={closeDayDemo} onTravel={travelDayDemo} onScripted={() => { singaporeHudState.scriptedDay = true }} />}</Suspense>
  ...
  const simulationNote = isNtuMapRebuild()
    ? (state.language === 'zh' ? '依据 NTUMap / Finute 校园模型进行方块化重建，覆盖教学区、宿舍、NIE 和体育设施。当前为室外场景，人物、交通与活动均为模拟。' : 'A voxel reconstruction of the NTUMap / Finute campus model, covering academic areas, residences, NIE and sports facilities. This is an exterior prototype; people, traffic and activities are simulated.')
    : localized(SINGAPORE_STORY.simulationNote)
  const mapCredits = <span className='sg-map-credits ao-interactive' onPointerDown={event => event.stopPropagation()} onClick={stopWalking}><a href='https://www.openstreetmap.org/copyright' target='_blank' rel='noopener noreferrer'>© OpenStreetMap contributors · ODbL</a><a href={ntuMapAsset('SOURCES.md')} target='_blank' rel='noopener noreferrer'>{text.mapSources} ↗</a></span>
  ...
  return <div className='ao-glasses sg-glasses' lang={state.language === 'zh' ? 'zh-CN' : 'en'} onMouseDown={event => { if ((event.target as HTMLElement).closest('.ao-interactive')) event.stopPropagation() }}>
    ...
    <header className='ao-hud-top'>
      ...
      <div className='ao-hud-tools ao-interactive'>
        <button className='ao-panel-button' onClick={openCampusMap}>{state.language === 'zh' ? '地图' : 'Map'}<kbd>M</kbd></button>
        <button className='ao-panel-button ao-walk-button' aria-pressed={state.walking} disabled={panelOpen || state.busy || state.observerMode !== 'walk'} onClick={toggleWalking}>{state.walking ? text.stop : text.walk}<kbd>R</kbd></button>
    ...
    {isNtuMapRebuild() && activeBot && <NtuRegionBanner player={activeBot} language={state.language} />}
    {!panelOpen && getLoadedHallPrototype() && <NtuHallPrototypeHud language={state.language} busy={state.busy} onShuttle={takeShuttle} error={state.error ? text[state.error] : undefined} />}
    {!panelOpen && isNtuMapRebuild() && <NtuCampusExploreHud language={state.language} stops={scene?.tourStops} busy={state.busy} error={state.error ? text[state.error] : undefined} onShuttle={takeShuttle} onGlasses={() => openSingaporePanel()}
      onDemo={openDayDemo} observerMode={state.observerMode} onObserver={mode => { void changeObserverMode(mode) }} />}
    {!panelOpen && isNtuMapRebuild() && activeBot && <NtuCompanionHud language={state.language} player={activeBot} target={state.target} focus={stationName(state.focus)} quiet={state.quiet} onOpen={() => openSingaporePanel()} />}
    {!panelOpen && !getLoadedHallPrototype() && !isNtuMapRebuild() && !state.quiet && <button className='ao-mission ao-interactive' onClick={() => openSingaporePanel()}><span className='ao-eyebrow'>NTU · {state.language === 'zh' ? `第 ${state.visit} 次访问` : `VISIT ${state.visit}`}</span><strong>{title}</strong><span>{description}</span><span className='ao-mission-progress'>{receipts.map((receipt, index) => <i key={index} className={receipt ? 'is-active' : ''} />)}</span>{!allDone && <small className='sg-focus'>{text.focus} · {stationName(state.focus)}</small>}</button>}
    {!panelOpen && activeBot && scene && <SingaporeMinimap player={activeBot} stations={scene.stations} tourStops={scene.tourStops} language={state.language} focus={state.focus} touch={Boolean(currentTouch)} onVisit={state.observerMode === 'walk' ? takeShuttle : undefined} />}
    ...
    {panelOpen && <div className='ao-panel-layer ao-interactive' onPointerDown={event => event.stopPropagation()} onMouseDown={event => event.stopPropagation()}><div
      ...
      <div className='ao-panel-content' role='tabpanel'>
        ...
        {state.tab === 'travel' && <><h3>{text.travelTitle}</h3><p className='ao-footnote'>{text.travelText}</p><div className='sg-station-list'>{scene?.stations.map(station => <button key={station.id} disabled={state.busy} className='sg-station' onClick={() => takeShuttle(station.id)}><span><strong>{localized(station.name)}</strong><small>{localized(SINGAPORE_STORY.stations[station.id])}</small></span><span>{text.travelButton} ↗</span></button>)}</div>{Boolean(scene?.tourStops?.length) && <><h3>{state.language === 'zh' ? '探索整个校园' : 'Explore the campus'}</h3><p className='ao-footnote'>{state.language === 'zh' ? '快速前往其他校区；更多楼宇和房间可从全校园地图搜索。' : 'Travel to another area; search the full campus map for more buildings and rooms.'}</p><button className='sg-secondary' onClick={() => { closePanel(false); showModal({ reactType: 'singapore-campus-map' }); alwaysOnMenuInput.suppressPause(); document.exitPointerLock?.() }}>{state.language === 'zh' ? '搜索全校园地图' : 'Search full campus map'} ↗</button><div className='sg-station-list'>{scene?.tourStops?.filter(stop => stop.mapVisible !== false).slice(0, 20).map(stop => <button key={stop.id} disabled={state.busy} className='sg-station' onClick={() => takeShuttle(stop.id)}><strong>{localized(stop.name)}</strong><span>{text.travelButton} ↗</span></button>)}</div></>}{state.error && <p className='ao-error' role='alert'>{text[state.error]}</p>}</>}
      ...
      <footer className='ao-panel-footer'><p><span>{localized(SINGAPORE_STORY.geographyNote)}</span>{simulationNote}{mapCredits}</p><button className='ao-continue' onClick={() => closePanel()}>{text.explore}<span>↗</span></button></footer>
    </div></div>}
  </div>
}
